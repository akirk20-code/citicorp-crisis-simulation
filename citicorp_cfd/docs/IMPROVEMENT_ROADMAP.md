...
**Purpose:** Detailed technical plan for addressing each limitation in ASSUMPTIONS.md
...
### 1.1 Add Citicorp Slanted Roof (COMPLETED)

**Status:** Implemented in `generators/generate_stl_nyc3d.py` — 45° sloped wedge from 244.15m to 278.9m peak.
Re-mesh with `generate_stl_nyc3d.py` to use the updated geometry.

**Original issue:** Tower was a rectangular box with flat top at 278.9m
...
### 1.2 Rotate Building 45° to Historical Orientation (COMPLETED)

**Status:** Implemented in `generators/generate_stl_nyc3d.py` — tower rotated 45° with correct face-midpoint stilt positions.

**Original issue:** Tower aligned N-S-E-W (cardinal directions)
...
### 4.1 Atmospheric Boundary Layer Inlet (COMPLETED — refine z0)

**Status:** ABL inlet implemented in `0/U` using `atmBoundaryLayerInletVelocity` (z0=0.1m, Uref=44.7 m/s at Zref=10m). See `docs/VELOCITY_ANALYSIS.md` for profile analysis. **Remaining:** Increase z0 from 0.1m (suburban) to ~1.0m (dense urban Manhattan).

**Original issue:** Uniform inlet (U = 44.7 m/s constant)

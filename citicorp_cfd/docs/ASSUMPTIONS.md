...
- **Inlet:** Atmospheric boundary layer profile (`atmBoundaryLayerInletVelocity`, z0=0.1m, Uref=44.7 m/s at Zref=10m). Produces ~75.8 m/s at roof height. See `docs/VELOCITY_ANALYSIS.md` for details.
- **Outlet:** Zero gradient (zeroGradient for all fields)
- **Ground:** No-slip wall (`noSlip`) — real case would benefit from roughWallFunction with z0~1.0m (urban)
...
- **ABL profile:** Log-law inlet with z0=0.1m (suburban roughness; urban Manhattan should be z0~1.0m)
- ⚠ Current z0=0.1m underestimates urban roughness — refine to z0=1.0m for research-grade results
...
### High Priority
1. ~~**Add slanted roof to tower**~~ — DONE (`generators/generate_stl_nyc3d.py`)
2. ~~**Rotate building 45°**~~ — DONE (`generators/generate_stl_nyc3d.py`)
3. **Simulate 45° quartering wind** (critical case from Morgenstern 1995)
4. **Mesh refinement study** (6M, 12M cells on Hopper HPC)

### Medium Priority
5. ~~**Atmospheric boundary layer profile**~~ — DONE (z0=0.1m in `0/U`; refine to z0=1.0m for urban)
